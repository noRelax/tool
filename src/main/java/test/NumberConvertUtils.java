...
public class NumberConvertUtils {
	...
	public static String convertLikeCount(Integer number) {
		...
		int beforePoint = number / TEN_THOUSANDS;
		int afterPoint = (number % TEN_THOUSANDS) / (TEN_THOUSANDS / 10);
		if (Objects.equals(0, afterPoint)) {
			return beforePoint + "万";
		}
	 ...
	 * @return
	 */
	public static String convertCommentCount(Integer number) {
		if (Objects.isNull(number) || number <= 0) {
			return "";
		}
	 ...
	 * 作品、关注、粉丝、获赞数转换
	 * A.数字为为0-9999，显示具体数字
	 * B.数字为10000~99999999显示为：1万~9999.9万，例：109000将显示为10.9万，109001将显示为10.9万
	 * C.数字大于99999999显示为1.x亿
	 * @param number
	 * @return
	 */
	public static String convertUserFollow(Long number) {
		if (Objects.isNull(number) || number <= 0) {
			return "";
		}
		...
		int unit = number < HUNDRED_MILLIONS ? TEN_THOUSANDS : HUNDRED_MILLIONS;
		String unitSymbol = number < HUNDRED_MILLIONS ? "万" : "亿";
		long beforePoint = number / unit;
		long afterPoint = (number % unit) / (unit / 10);

		if (Objects.equals(0L, afterPoint)) {
			return beforePoint + unitSymbol;
		} else {
			return beforePoint + "." + afterPoint + unitSymbol;
		}
